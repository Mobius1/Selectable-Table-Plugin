 ...
 *
 */
window.Selectable&&"function"==typeof window.Selectable&&(Selectable.prototype.table=function(a){const b=this.version.split(".").map(c=>parseInt(c,10));if(1>b[0]&&11>b[1])return console.warn("The table plugin requires Selectable v0.11.0 and above.");if(void 0===a&&(a=this.get(0).node.closest("table")),a){const c=a.tHead.rows[0].cells,d=Array.from(a.tBodies[0].rows);a.addEventListener("click",g=>{const h=g.target;if("TH"===h.nodeName||"TD"===h.nodeName){g.preventDefault();let j;if(a){const k=h.dataset;if("selectable"in k){if("all"===h.dataset.selectable)j=this.getItems();else if("TD"===h.nodeName&&"row"===k.selectable){const l=h.closest("tr");l&&(j=Array.from(l.cells))}else"TH"===h.nodeName&&"column"===k.selectable&&(j=Array.from(a.tBodies[0].rows).map(l=>{return l.cells[h.cellIndex]}));h.selected?(this.unselect(j),h.selected=!1):(this.select(j),h.selected=!0)}}}}),this.on("selectable.end",g=>{const h=g.target.closest("td"),j=g.target.closest("th");let k=0;if(j&&"column"===j.dataset.selectable){for(let l=0;l<d.length;l++)this.get(d[l].cells[j.cellIndex]).selected&&k++;j.selected=k===d.length}else if(h&&"row"===h.dataset.selectable){for(let l=0;l<c.length;l++)this.get(d[h.rowIndex].cells[l]).selected&&k++;h.selected=k===c.length}})}});